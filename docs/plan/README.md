...
> 增量纪律：M7/M8 是 M6 收口后的纯增量扩展（多库/多用户），M4-M6 期间不留半成品扩展
...
| M4 体验 | [M4-体验升级方案.md](M4-体验升级方案.md) | 📋 待开发 | 溯源 UI + Redis 持久记忆 + 意图识别（独立小模型可配）+ 前端四模式 |
...
| M8 多用户 | [M8-多用户方案.md](M8-多用户方案.md) | 📋 待开发（M7 后） | Sa-Token 鉴权 + owner 归属隔离 + 登录态前端 |

## 依赖关系与执行顺序

```
M1 ✅ → M2 ✅ → M3 ✅ ──┬─▶ M4 (体验+意图) ─▶ M5 (多 Agent) ─▶ M6 (评测+部署收口)
                         │                                        └─▶ M7 (多知识库) ─▶ M8 (多用户)
```

- M4/M5 依赖 M3 的模式分流；M6 收口依赖全部；M7/M8 是收口后的纯增量（多用户依赖多库存在，顺序不可逆）
